#Version 8
#BeginDescription
This script is designed to create three types of joints—Simple Scarf, Double Scarf, and Lap—on selected beams. It first divides the beam(s) at a specified point and then applies the chosen joint. Users can define the joint's length and depth. An "Offset from Axis" setting allows the main joint cut to be shifted vertically. The script also supports adding through-drills and sinkholes (counterbores) on the top and bottom surfaces; these drilling features remain aligned with the original beam axis, regardless of any offset applied to the main joint. The user interface features clearly named properties for easy configuration.
...
version  value="1.7" date="05mar20" author="david.delombaerde@hsbcad.com"> setting boundaries when moving the grippoint. And resetting the value when grip is out of bounds. 
